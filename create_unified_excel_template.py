#!/usr/bin/env python3
"""
Unified Excel Template Generator
===============================
Creates a comprehensive Excel template with all required sheets:
- SMOKE: Original smoke test configurations
- CONTROLLER: Test suite controller configuration 
- DATAVALIDATIONS: Enhanced data validation tests with proper table parameters
- INSTRUCTIONS: Comprehensive usage instructions
...
def create_unified_excel_template():
    """Create unified Excel template with all required sheets"""
    
    print("🔧 CREATING UNIFIED EXCEL TEMPLATE")
    print("=" * 50)
    ...
    # Save the workbook
    filename = "unified_sdm_test_suite.xlsx"
    wb.save(filename)
    
    print(f"✅ Created unified Excel template: {filename}")
    print()
    ...
    print("-" * 30)
    print("   ✅ SMOKE sheet: Original smoke test configurations")
    ...
    print("   ✅ Proper table parameter configurations")
    
    return filename


def create_smoke_sheet(wb):
    """Create SMOKE sheet with original smoke test configurations"""
    ...
    # Add smoke test data
    for row_idx, test_data in enumerate(smoke_tests, 2):
...
def create_instructions_sheet(wb):
    ...
    # Title
    ws.cell(row=1, column=1, value="UNIFIED SDM TEST SUITE - COMPREHENSIVE INSTRUCTIONS")
    ...
    instructions = [
        "",
        "📋 OVERVIEW:",
        "This unified Excel template contains comprehensive test configurations for the Multi-Database Data Validation Framework.",
        "All sheets work together to provide complete testing coverage across different test categories.",
        "",
        "📂 SHEET DESCRIPTIONS:",
        "",
        "🔹 SMOKE Sheet:",
        "   • Contains basic smoke tests for environment validation",
        "   • Tests connectivity, configuration, and basic functionality",
        "   • Execute first to verify environment readiness",
        "   • Categories: SETUP, CONFIGURATION, SECURITY, CONNECTION, QUERIES, PERFORMANCE",
        "",
        "🔹 CONTROLLER Sheet:",
        ...
        "• Test_Case_ID: Unique identifier for the test (e.g., SMOKE_001, DVAL_001)",
        ...
        "   python main.py --sheet SMOKE",
        "",
        "2. Execute data validation tests:",
        "   python execute_enhanced_data_validation_tests.py unified_sdm_test_suite.xlsx",
        "",
        "3. Execute specific test category:",
        "   python main.py --category SCHEMA_VALIDATION",
        "",
        "4. Validate Excel template:",
        "   python enhanced_excel_validator.py unified_sdm_test_suite.xlsx",
        "",
        "✅ BEST PRACTICES:",
        "",
...
def create_reference_sheet(wb):
    ...
    # Title
    ws.cell(row=1, column=1, value="REFERENCE DOCUMENTATION")
    ws.cell(row=1, column=1).font = Font(bold=True, size=16, color="FFFFFF")
    ...
    reference_data = [
        ...
        "TABLE_EXISTS", "_execute_table_exists_test", "Verify table existence", "Any",
        "TABLE_SELECT", "_execute_table_select_test", "Test table SELECT operations", "Any",
        "TABLE_ROWS", "_execute_table_rows_test", "Validate table row operations", "Any",
        "TABLE_STRUCTURE", "_execute_table_structure_test", "Analyze table structure", "Any",
    ...
    for row_idx, ref_data in enumerate(reference_data, 2):
        if isinstance(ref_data, str):
            ...
            for col_idx, value in enumerate(ref_data, 1):
                ...
                if row_idx == 4 or row_idx == 20 or row_idx == 27 or row_idx == 32:  # Header rows
                    cell.font = Font(bold=True, color="FFFFFF")
...
def main():
    ...
    filename = create_unified_excel_template()
    
    print("🎯 USAGE INSTRUCTIONS:")
    print("-" * 30)
    print(f"   1. Use '{filename}' as your master test suite")
    print("   2. Execute smoke tests: python main.py --sheet SMOKE")
    print("   3. Execute data validation: python execute_enhanced_data_validation_tests.py")
    print("   4. Validate template: python enhanced_excel_validator.py")
    print()
    print("✅ UNIFIED EXCEL TEMPLATE CREATION COMPLETE!")
