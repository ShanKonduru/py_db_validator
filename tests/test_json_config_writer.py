...
class TestJsonConfigWriter:
    ...
    def test_add_environment(self, sample_config):
        ...
        updated_config = JsonConfigWriter.add_environment(
            sample_config.copy(), "QA", applications
        )
    ...
    def test_add_environment_existing(self, sample_config):
        ...
        applications = {
            "RED": DatabaseConnection(
                db_type="oracle", host="test.com", port=1521, database="test"
            )
        }
    ...
    def test_add_application(self, sample_config, sample_connection):
        ...
        updated_config = JsonConfigWriter.add_application(
            sample_config.copy(), "DEV", "TPS", sample_connection
        )
    ...
    @pytest.mark.unit
    @pytest.mark.negative
    def test_add_application_environment_not_found(
        self, sample_config, sample_connection
    ):
        """Test adding application to non-existent environment"""
        with pytest.raises(KeyError, match="Environment 'NONEXISTENT' not found"):
            JsonConfigWriter.add_application(
                sample_config, "NONEXISTENT", "TPS", sample_connection
            )
    ...
    def test_add_application_existing(self, sample_config, sample_connection):
        ...
        with pytest.raises(
            ValueError, match="Application 'RED' already exists in environment 'DEV'"
        ):
            JsonConfigWriter.add_application(
                sample_config, "DEV", "RED", sample_connection
            )
    ...
    def test_add_application_invalid_connection_type(self, sample_config):
        ...
        with pytest.raises(
            ValueError, match="Connection must be a DatabaseConnection object"
        ):
            JsonConfigWriter.add_application(
                sample_config, "DEV", "TPS", "not a connection"
            )
    ...
    def test_add_application_empty_name(self, sample_config, sample_connection):
        ...
        with pytest.raises(ValueError, match="Application name cannot be empty"):
            JsonConfigWriter.add_application(
                sample_config, "DEV", "", sample_connection
            )
    ...
    def test_remove_application(self, sample_config, sample_connection):
        ...
        config = sample_config.copy()
        config = JsonConfigWriter.add_application(
            config, "DEV", "TPS", sample_connection
        )
    ...
    def test_add_application_invalid_data(self, sample_connection):
        ...
        with pytest.raises(ValueError, match="Configuration data must be a dictionary"):
            JsonConfigWriter.add_application(
                "not a dict", "DEV", "TPS", sample_connection
            )
    ...
    def test_add_environment_invalid_application_config(self, sample_config):
        ...
        applications = {"TPS": "not a dict or connection"}

        with pytest.raises(
            ValueError, match="Invalid application configuration for 'TPS'"
        ):
